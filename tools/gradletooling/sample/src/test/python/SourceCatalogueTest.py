    ...
    def setUpClass(cls):
        ...
        cls.sc = SourceCatalogue( name = "testCat",
           entry = [SDSSSource(name = "testSource",classification = SourceClassification.AGN,
                               position = SkyCoordinate(frame = frame,latitude =  RealQuantity(52.5, degree ),longitude =  RealQuantity(2.5, degree )),
           positionError = ellipseError, #note subsetting forces compile need AlignedEllipse
            luminosity = [
            LuminosityMeasurement(description = "lummeas",type=LuminosityType.FLUX,
                                  value=RealQuantity(2.5, jansky ),error =  RealQuantity(.25, jansky ),
                             filter = PhotometryFilter(bandName ="C-Band",spectralLocation =  
                                      RealQuantity(5.0,GHz),dataValidityFrom =  datetime.now(),
                                      dataValidityTo =  datetime.now(),description = "radio band",name = "C-Band measure")),
            LuminosityMeasurement(description = "lummeas2", type = LuminosityType.FLUX,
                                   value =  RealQuantity(3.5, jansky ), error =  RealQuantity(.25, jansky ),
            filter = PhotometryFilter(bandName ="L-Band",
                                    spectralLocation =  RealQuantity(1.5,GHz),
                                    dataValidityFrom =  datetime.now(),
                                    dataValidityTo =  datetime.now(),description = "radio band",name = "L-Band"))
            ]
