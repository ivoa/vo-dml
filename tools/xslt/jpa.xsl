...
//      @jakarta.persistence.Access(jakarta.persistence.AccessType.FIELD)
    <xsl:choose>
        ...
        <xsl:otherwise>
            <xsl:text>@jakarta.persistence.Embeddable</xsl:text>&cr;
            <xsl:value-of select="concat('@jakarta.persistence.DiscriminatorValue(',$dq,$vodml-ref,$dq,' )')"/>&cr;
        </xsl:otherwise>
    </xsl:choose>
  ...
  For dataType attributes we (attempt to) use embedded types. -->
    <xsl:template match="attribute" mode="JPAAnnotation">
        ...
        @jakarta.persistence.Column( name = "<xsl:apply-templates select="." mode="columnName"/>", nullable = <xsl:apply-templates select="." mode="nullable"/>
                                ...
                                <xsl:if test="vf:findmapping(datatype/vodml-ref,'java')/@jpa-converter">
        @jakarta.persistence.Convert(converter=<xsl:value-of select="vf:findmapping(datatype/vodml-ref,'java')/@jpa-converter"/>.class)
                                </xsl:if>

                            </xsl:when>
                            <xsl:otherwise>
        //primitive generated by vodml-tools - will be a class with a "value" member
        @jakarta.persistence.Embedded
                             <xsl:if test="current()/parent::*/name()= 'objectType'">
...
<!--   this was main <xsl:template match="att[not(*)]|att[count(descendant::*)=1 and dt]" mode="doAttributeOverride">-->
