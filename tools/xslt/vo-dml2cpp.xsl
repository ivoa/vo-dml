...
-->
  ...
  <!-- main pattern : processes for root node model -->
    <xsl:template match="/">
  <xsl:message >Generating C++ - considering models <xsl:value-of select="string-join($models/vo-dml:model/name,' and ')" /></xsl:message>
  ...
  <xsl:template match="vo-dml:model" mode="intro">
    <xsl:message>
-------------------------------------------------------------------------------------------------------
-- Generating C++ code for model <xsl:value-of select="name"/> [<xsl:value-of select="title"/>].
...
<!--      </xsl:apply-templates>-->
  ...
  <xsl:template match="vo-dml:model|package" mode="content">
      ...
      <xsl:variable name="file" select="concat($root_package_dir, '/',string-join(./ancestor-or-self::*/name,'_') , '.h')"/>
