 ...
 <!-- main pattern : processes for root node model -->
  ...
  <xsl:template match="vo-dml:model">
    ...
    <xsl:variable name="elformdefault">
      <xsl:choose>
        <xsl:when test="vf:XMLqualified($modelname)"><xsl:value-of select="'qualified'"/></xsl:when>
    ...
    <xsl:message >Generating XSD for  <xsl:value-of select="$modelname "/> - considering models <xsl:value-of select="string-join($models/vo-dml:model/name,', ')" /></xsl:message>
      ...
      <xsd:schema  xmlns:vc="http://www.w3.org/2007/XMLSchema-versioning"
                  vc:minVersion="1.1"> <!--FIXME cannot get a 1.1 schema validator to work actually -->
        ...
        <xsd:element>
          ...
          <xsd:complexType>
            <xsd:sequence>
              <xsd:element name="refs" minOccurs="0"> <!-- would really like this to be in default namespace - however targetNamespace="" is a xsd 1.1 feature -->
                <xsd:complexType>
